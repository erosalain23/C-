...
namespace Carrosse
{
    class MaBalle : MonPoint
    {
        #region Données membres
        private Color _Pot = Color.Yellow;
        private bool _Remplir = true;
        private int _Rayon = 1;
        #endregion

        #region Constructeurs
        public MaBalle(PictureBox hebergeur, int xc, int yc, int rayon) : base(hebergeur, xc, yc)
        {
            Rayon = rayon;
        }

        public MaBalle(PictureBox hebergeur, int xc, int yc, int rayon, Color pot) : base(hebergeur, xc, yc)
        {
            Rayon = rayon;
            Pot = pot;
        }

        public MaBalle(PictureBox hebergeur, int xc, int yc, int rayon, Color crayon, Color pot) : base(hebergeur, xc, yc, crayon)
        {
            Rayon = rayon;
            Pot = pot;
        }
